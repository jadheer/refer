<?php 
class ES_Location extends ESSource 
{
	var $index;
	var $type;
		...
		try{
			...
			if ($objects['hits']['total']>0):
				...
				if (!empty($object['_source'])):
					...
					$robject->location_id	=	(int)$row->location_id;
					$robject->geography_id	=	(int)$row->geography_id;
	...
	public function getLocGeoIds($abbreviation){
		try{
			$params = [];
			$params['from']  	= 	0;
			$params['size']  	= 	1;
			$params['index'] 	= 	$this->index;
			$params['body']['query']['bool']['must']	=	['match'	=>	['abbreviation' => $abbreviation]];
