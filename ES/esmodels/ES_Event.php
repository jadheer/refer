<?php 
class ES_Event extends ESSource 
{
	var $index;
	var $type;
	...
	public function getEventsByType($object) {
		try{
			$params = [];
			...
			if ($object->type=="past"):
				$params['body']['sort']	=	[['start_datetime' => ['order'	=>	'desc']], ['event_id' => ['order' => 'asc']]];
			else:
				$params['body']['sort']	=	[['start_datetime' => ['order'	=>	'asc']], ['event_id' => ['order' => 'asc']]];
			endif;
	...
	public function getUniqueEventDatesByType($type) {
		try{
			$params = [];
			$params['index'] 	= 	$this->index;
	...
	public function getEventByDate($object) {
		try{
			$params = [];
			$params['from']		=	(int)$object->start??0;
			$params['size']  	= 	(int)$object->end??1000;
	...
	public function getEventById($object){
		try{
			$params = [];
			$params['index'] 	= 	$this->index;
			$params['type']		=	$this->type;
			$params['id']		=	(int)$object->event_id;
	...
	private function setObject($row, $object, $componentMapping){
		try{
			...
			$robject->event_id		=	(int)$row->event_id;
			...
			$robject->promo_image 	=	(object)[	'base_url'		=>	(string)_AWS_URL._EVENTS_IMAGES_DIR,
			...
			$robject->gallary	=	(object)[	'base_url'	=>	(string)_AWS_URL._EVENTS_IMAGES_DIR,
												'pictures'	=>	$pictures];

			$allowedRegistration		=	(boolean)(!empty($row->registrations)?true:false);
			if ($allowedRegistration):
				$allowedRegistration 	=	(boolean)(strtotime($robject->end_date.' '.$robject->end_time)>time())?true:false;
				if (strtotime($row->registrations_last_date) < strtotime(date('Y-m-d'))):
					$allowedRegistration	=	(boolean)false;
				endif;
				if ($allowedRegistration):
					$allowedRegistration	=	(boolean)($row->max_registrations==count($row->registrations_details))?false:true;
				endif;
			endif;
			...
			$robject->event_location		=	(string)$this->common->entityDecode($row->event_location);
