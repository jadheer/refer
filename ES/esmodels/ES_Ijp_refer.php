<?php 
class ES_Ijp_refer extends ESSource 
{
	var $index;
	var $type;
	var $stages;
	...
	public function getEmployeeReferrals($object){
		try{
			$params = [];
	...
	private function setOutput($results, $object){
		try {
			...
			foreach ($robjects as $row):
				...
				$robject->title     			= 	(string)isset($ijp_arr[$index]->title)?(string)$this->common->entityDecode($ijp_arr[$index]->title):'';
				$robject->indent_number			= 	(string)isset($ijp_arr[$index]->indent_number)?$ijp_arr[$index]->indent_number:'';
				$robject->indent_status			= 	(string)isset($ijp_arr[$index]->indent_status)?$ijp_arr[$index]->indent_status:'';
				...
				$robject->candidate_email		= 	(string)$row->email;
				$robject->mobile_number			= 	(string)$row->mobile;
				...
				$robject->linkedin_url			= 	(string)$row->linkedin_url;
				$robject->candidate_number		= 	(string)$row->candidate_number;
				$robject->employee_id			= 	(int)$row->employee_id;
				$robject->referred_on  			= 	(string)$row->created_on;
	...
	public function update($referral){
		...
		{
			$params 		= 	[];
			$params['body']	= 	[	'doc' 		=>	[	'ijp_referral_id'			=>	(int)$referral->ijp_referral_id,
														'ijp_id'					=>	(int)$referral->ijp_id,
														'employee_id'				=>	(int)$referral->employee_id,
														'candidate_number'			=>	(string)$referral->candidate_number,
														'linkedin_url'				=>  (string)$referral->linkedin_url,
														'first_name'				=>	(string)$this->common->entityEncode($referral->first_name),
														'last_name'					=>	(string)$this->common->entityEncode($referral->last_name),
														'email'						=>	(string)$referral->email,
														'mobile'					=>	(string)$referral->mobile,
														'stage'						=>	(int)$referral->stage,
														'status'					=>	(int)$referral->status,
														'created_on'				=>	(string)$referral->created_on,
														'updated_on'				=>	(string)$referral->updated_on,
														'enabled'					=>	(int)$referral->enabled
													],
					'doc_as_upsert'	=>	true];
			
			$params['index'] = 	$this->index;
			$params['type']  = 	$this->type;
			$params['id']    = 	(int)$referral->ijp_referral_id;
	...
	public function insert($referral){
		...
		{
			$params 		= 	[];
			$params['body']	= 	[		'ijp_referral_id'			=>	(int)$referral->ijp_referral_id,
										'ijp_id'					=>	(int)$referral->ijp_id,
										'employee_id'				=>	(int)$referral->employee_id,
										'candidate_number'			=>	(int)isset($referral->candidate_number)?$referral->candidate_number:"",
										'linkedin_url'				=>  (string)$referral->linkedin_url,
										'first_name'				=>	(string)$this->common->entityEncode($referral->first_name),
										'last_name'					=>	(string)$this->common->entityEncode($referral->last_name),
										'email'						=>	(string)$referral->email,
